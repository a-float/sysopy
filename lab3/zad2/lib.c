...
void merge_file_sequence(Table *table, Sequence* seq, int start) {
    ...
    char *files[2];
    char *pair_copy = calloc(sizeof(char), MAX_PAIR_SIZE);
    for (int i = start; i < seq->size; i++) {
        if (table->blocks[i]) delete_block(table, i);
        strcpy(pair_copy, seq->pairs[i]);
        files[0] = strtok(pair_copy, ":");
        files[1] = strtok(NULL, ":");
        // printf("Merging file %s and file %s\n",files[0], files[1]);
        #ifdef forked_merge
        if(fork() == 0){
        #endif
            //merge is performed but te result is not used, as it would need to be saved to a file
            //which would heavily impact the time measurement
            Block *block = merge_files(files[0], files[1]);
            //free memory
            if (block != NULL) {
...
int add_file_pair(struct Sequence* seq, char* pair){
    ...
    seq->pairs[seq->current_index] = calloc(sizeof(char), strlen(pair)+1);
